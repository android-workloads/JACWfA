...
/**
 * Non-recursive Qsort implementation for [integer] arrays.
 *  
 ...
 * 
 * 
 * @see com.intel.JACW.sort.QSortRecursiveI
 */
public class QSortIterativeI extends AbstractTestCase {
	 ...
	 */
	public void qsort(int[] array) throws TestRuntimeErrorException {
		if (array.length > 0) {
			int left = 0;
			...
			while (top >= 0) {
				...
				right = stack[top--];
				left = stack[top--];
				// set pivot element at its place in sorted array
