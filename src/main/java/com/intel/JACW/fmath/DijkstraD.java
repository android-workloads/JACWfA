...
/**
 * <p> Classic implementation of Dijkstra algorithm [double].</p>
 ...
 * </ul>
 */
public class DijkstraD extends AbstractTestCase {
	...
	private final double distUnreachable = Long.MAX_VALUE >> 1;
	...
	public void init(RunConfig config) throws InvalidTestFormatException {
		...
		try {
			...
			weights = new double[cntVertices][cntVertices];
			for (int row = 0; row < cntVertices; row++) {
				s = in.readLine();
				StringTokenizer st = new StringTokenizer(s);
				for (int col = 0; col < cntVertices; col++) {
					weights[row][col] = Double.parseDouble(st.nextToken());
			...
			in.close();
			distance = new double[cntVertices];
