...
/**
 * <p>Test solves system of 2 differential equations using Runge-Kutta method [float].</p>
 ...
 * 
 */
public class RungeKutta2F extends AbstractTestCase {
	...
	public long iteration() {
		...
		for (int i = 0; i < repeats; i++) {
			...
			for (int j = 0; j < MAX_ITERS; j++) {
				count++;
				float ax1 = fx1(t, x1, x2);
				float ax2 = fx2(t, x1, x2);

				float bx1 = fx1(t + halfH, x1 + halfH * ax1, x2 + halfH * ax2);
				float bx2 = fx2(t + halfH, x1 + halfH * ax1, x2 + halfH * ax2);

				float cx1 = fx1(t + halfH, x1 + halfH * bx1, x2 + halfH * bx2);
				float cx2 = fx2(t + halfH, x1 + halfH * bx1, x2 + halfH * bx2);

				float dx1 = fx1(t + h, x1 + h * cx1, x2 + h * cx2);
				float dx2 = fx2(t + h, x1 + h * cx1, x2 + h * cx2);
	...
	@Override
	public void done() {
		Resources.writeHelperBuffer(RungeKutta2F.class, String.valueOf(accumulator));
	}

	private static float fx1(float t, float x1, float x2) {
		return t + 5*x1 + x2;
	}

	private static float fx2(float t, float x1, float x2) {
		return t + x1 + 5*x2;
	}
}
