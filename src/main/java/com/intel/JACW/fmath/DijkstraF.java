...
/**
 * <p> Classic [float] implementation of Dijkstra algorithm.</p>
 ...
 * </ul>
 */
public class DijkstraF extends AbstractTestCase {
	...
	private final float distUnreachable = Long.MAX_VALUE >> 1;
	...
	public void init(RunConfig config) throws InvalidTestFormatException {
		...
		try {
			...
			weights = new float[cntVertices][cntVertices];
			for (int row = 0; row < cntVertices; row++) {
				s = in.readLine();
				StringTokenizer st = new StringTokenizer(s);
				for (int col = 0; col < cntVertices; col++) {
					weights[row][col] = Float.parseFloat(st.nextToken());
			...
			in.close();
			distance = new float[cntVertices];
