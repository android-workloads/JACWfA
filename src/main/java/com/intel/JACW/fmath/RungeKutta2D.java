...
/**
 * <p>Test solves system of 2 differential equations using Runge-Kutta method [double].</p>
 ...
 * 
 */
public class RungeKutta2D extends AbstractTestCase {
	...
	public long iteration() {
		...
		for (int i = 0; i < repeats; i++) {
			...
			for (int j = 0; j < MAX_ITERS; j++) {
				count++;
				double ax1 = fx1(t, x1, x2);
				double ax2 = fx2(t, x1, x2);

				double bx1 = fx1(t + halfH, x1 + halfH * ax1, x2 + halfH * ax2);
				double bx2 = fx2(t + halfH, x1 + halfH * ax1, x2 + halfH * ax2);

				double cx1 = fx1(t + halfH, x1 + halfH * bx1, x2 + halfH * bx2);
				double cx2 = fx2(t + halfH, x1 + halfH * bx1, x2 + halfH * bx2);

				double dx1 = fx1(t + h, x1 + h * cx1, x2 + h * cx2);
				double dx2 = fx2(t + h, x1 + h * cx1, x2 + h * cx2);
	...
	@Override
	public void done() {
		Resources.writeHelperBuffer(RungeKutta2D.class, String.valueOf(accumulator));
	}

	private static double fx1(double t, double x1, double x2) {
		return t + 5*x1 + x2;
	}

	private static double fx2(double t, double x1, double x2) {
		return t + x1 + 5*x2;
	}
}
