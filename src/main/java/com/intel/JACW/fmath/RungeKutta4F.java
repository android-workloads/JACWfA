...
/**
 * <p>Test solves system of 4 differential equations using Runge-Kutta method [float].</p>
 ...
 * 
 */
public class RungeKutta4F extends AbstractTestCase {
	...
	public long iteration() {
		...
		for (int i = 0; i < repeats; i++) {
			...
			for (int j = 0; j < MAX_ITERS; j++) {
				count++;
				float ax1 = fx1(t, x1, x2, x3, x4);
				float ax2 = fx2(t, x1, x2, x3, x4);
				float ax3 = fx3(t, x1, x2, x3, x4);
				float ax4 = fx4(t, x1, x2, x3, x4);

				float bx1 = fx1(t + halfH, x1 + halfH * ax1, x2 + halfH * ax2, x3 + halfH * ax3, x4 + halfH * ax4);
				float bx2 = fx2(t + halfH, x1 + halfH * ax1, x2 + halfH * ax2, x3 + halfH * ax3, x4 + halfH * ax4);
				float bx3 = fx3(t + halfH, x1 + halfH * ax1, x2 + halfH * ax2, x3 + halfH * ax3, x4 + halfH * ax4);
				float bx4 = fx4(t + halfH, x1 + halfH * ax1, x2 + halfH * ax2, x3 + halfH * ax3, x4 + halfH * ax4);

				float cx1 = fx1(t + halfH, x1 + halfH * bx1, x2 + halfH * bx2, x3 + halfH * bx3, x4 + halfH * bx4);
				float cx2 = fx2(t + halfH, x1 + halfH * bx1, x2 + halfH * bx2, x3 + halfH * bx3, x4 + halfH * bx4);
				float cx3 = fx3(t + halfH, x1 + halfH * bx1, x2 + halfH * bx2, x3 + halfH * bx3, x4 + halfH * bx4);
				float cx4 = fx4(t + halfH, x1 + halfH * bx1, x2 + halfH * bx2, x3 + halfH * bx3, x4 + halfH * bx4);

				float dx1 = fx1(t + h, x1 + h * cx1, x2 + h * cx2, x3 + h * cx3, x4 + h * cx4);
				float dx2 = fx2(t + h, x1 + h * cx1, x2 + h * cx2, x3 + h * cx3, x4 + h * cx4);
				float dx3 = fx3(t + h, x1 + h * cx1, x2 + h * cx2, x3 + h * cx3, x4 + h * cx4);
				float dx4 = fx4(t + h, x1 + h * cx1, x2 + h * cx2, x3 + h * cx3, x4 + h * cx4);
	...
	@Override
	public void done() {
		Resources.writeHelperBuffer(RungeKutta4F.class, String.valueOf(accumulator));
	}

	private static float fx1(float t, float x1, float x2, float x3, float x4) {
		return t + 5*x1 + x2;
	}

	private static float fx2(float t, float x1, float x2, float x3, float x4) {
		return t + x1 + 5*x2 + x3;
	}

	private static float fx3(float t, float x1, float x2, float x3, float x4) {
		return t + x2 + 5*x3 + x4;
	}

	private static float fx4(float t, float x1, float x2, float x3, float x4) {
		return t + x3 + 5*x4;
	}
}
