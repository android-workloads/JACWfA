...
/**
 * <p> Classic [integer] implementation of Dijkstra algorithm.</p>
 ...
 * </ul>
 */
public class DijkstraI extends AbstractTestCase {
	...
	private final int distUnreachable = Integer.MAX_VALUE >> 1;
	...
	public void init(RunConfig config) throws InvalidTestFormatException {
		...
		try {
			...
			weights = new int[cntVertices][cntVertices];
			for (int row = 0; row < cntVertices; row++) {
				s = in.readLine();
				StringTokenizer st = new StringTokenizer(s);
				for (int col = 0; col < cntVertices; col++) {
					weights[row][col] = Integer.parseInt(st.nextToken());
